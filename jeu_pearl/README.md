...
- **CPLEX Community Edition** : timeout fréquent au-delà de `N=14` à cause de
  la limite des 1000 variables.
- **`generateInstance(n)`** est aléatoire (pas de seed) ; commit `data/` si
  vous voulez figer le dataset utilisé pour le rapport.
- **`heuristicSolve`** réussit principalement sur les instances très
  contraintes par les bords ; la version actuelle ne fait pas de backtracking.
...
|----------|---------------|-----|
| `ERROR: CPLEX not properly installed` | env `CPLEX_STUDIO_BINARIES` mal positionnée | `source ~/.bashrc` puis `julia -e 'using Pkg; Pkg.build("CPLEX")'` |
| `SystemError: Aucun espace disponible` | disque plein durant `Pkg.add` | nettoyer les caches (`rm -rf ~/.julia/artifacts ~/.julia/scratchspaces ~/.julia/compiled`) puis relancer |
| `displaySolution` plante avec "Result index out of bounds" | l'instance est infaisable, `isFeas == false` | toujours tester `isFeas` avant — utiliser `solveAndDisplay(t)` qui le fait |
| Avertissements de "method redefinition" au lancement | les fichiers `*.jl` font chacun leur `include("io.jl")` — inoffensif | les ignorer |

---

## 12. Auteurs / Crédits

Projet réalisé dans le cadre du module **MOPSI / Recherche Opérationnelle**
de l'**ENSTA**. Architecture inspirée du squelette `sudoku1.0/` fourni par
les enseignants.

Modélisation, callbacks, heuristique et visualisation : KENFACK FRANCK.
